...
/*   Updated: 2022/09/02 18:17:00 by bgenie           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_philo_datas   *init_philo_datas(t_datas *datas, t_philo *philo)
{
    ...
    philo_datas->writer = datas->writer;
    return (philo_datas);
}
...
static pthread_mutex_t *init_writer(void)
{
    pthread_mutex_t *writer;

    writer = (pthread_mutex_t *) malloc(sizeof(pthread_mutex_t));
    if (!writer)
        return (NULL);
    if (pthread_mutex_init(writer, NULL) != 0)
        return (NULL);
    return (writer);
}
...
t_datas *init_datas(int argc, char **args)
{
    ...
    datas->writer = init_writer();
    if (!datas->forks || !datas->writer)
        return (NULL);
